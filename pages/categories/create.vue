    ...
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-gray-500 mt-1">Define master template structure.</p>
        </div>
        <NuxtLink to="/categories" class="btn-secondary">Cancel</NuxtLink>
      </div>

      <form @submit.prevent="saveCategory" class="max-w-4xl mx-auto space-y-8">
        <!-- Basic Info -->
        <div
...
const saveCategory = async () => {
  ...
  try {
    ...
    alert("Failed to save category. Check permissions.");
  } finally {
    saving.value = false;
  }
};
